<script lang="ts">
  ...
  onMount(() => {
    ...
    EventsOn('pty-output', (data: any) => {
      if (terminal && data) {
        const raw = typeof data === 'string' ? data : (typeof data.text === 'string' ? data.text : '');
        if (raw) {
          terminal.write(raw);
          logText += raw;
        }
    ...
    return () => {
      ...
      EventsOff('pty-output'); EventsOff('claude-done');
      EventsOff('claude-stuck'); EventsOff('claude-status'); EventsOff('claude-error');
    };
  });
  ...
  async function handleSend(text: string) {
    ...
    catch (e) { console.error('Send failed:', e); status = 'error'; }
  }
  ...
  async function handleToggleMode() {
    ...
    const newSettings = { ...settings, active_mode: nextMode, custom_provider_mode: customProviderMode };
    try {
      await updateSettings(newSettings);
      settings = newSettings;
      terminal?.clear();
      await restartClaude();
    } catch (e) { console.error('Toggle custom provider mode failed:', e); status = 'error'; }
  }

  async function handleModeChange(event: CustomEvent<string>) {
    const mode = event.detail;
    try { await setActiveMode(mode); }
    catch (e) { console.error('Mode switch failed:', e); }
  }

  async function handleRestart() {
    try {
      status = 'idle'; stuckVisible = false;
      terminal?.clear();
      await restartClaude();
    } catch (e) { console.error('Restart failed:', e); status = 'error'; }
  }
  ...
  async function handleClearHistory() {
    try {
      await clearHistory();
      terminal?.clear();
      stuckVisible = false; status = 'idle';
  ...
  async function handleTerminalResize(event: CustomEvent<{cols: number; rows: number}>) {
    try { const { cols, rows } = event.detail; await resizeTerminal(cols, rows); } catch {}
  }
  ...
  function handleKeydown(event: KeyboardEvent) {
    if (event.altKey) {
      switch (event.key) {
        case '1': event.preventDefault(); handleShiftTab(); break;
        case '2': event.preventDefault(); handleCtrlO(); break;
        case '3': event.preventDefault(); handleUp(); break;
        case '4': event.preventDefault(); handleDown(); break;
      }
    }
  }
</script>
...
<InputBar bind:value={inputText} disabled={status === 'responding'}
  on:send={(e) => handleSend(e.detail)} on:settings={() => (settingsOpen = true)} />
{#if settingsOpen}
